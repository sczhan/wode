...
# 游戏中各部件图像所在路径及各部键
image_path = os.getcwd() + os.path.join("/img/")   # ?
filename_suffix = ".gif"     # ?
...
# 天空 - 将图片的右下角显示在画布的(480, 600) 的做坐标处
#  如下部件以相同的方式进行显示
...
# 敌军小飞机 - x轴上方开始向下移动, 具体从x轴那个点开始是随机的
initial_anchor_smallplane = tk.SW
initial_anchor_smallplane_x = rd.randrange(0,window_boundary_col - image_smallplane_width)
initial_anchor_smallplane_y = 0

# 敌军大飞机 - x轴上方开始向下移动,具体从x轴的那个点开始是随机的
initial_anchor_bigplane = tk.SW
initial_anchor_bigplane_x = rd.randrange(0, window_boundary_col - image_bigplane_width)
initial_anchor_bigplane_y = 0

# 敌军小蜜蜂 - x轴上方向下开始移动,具体从x轴的那个点开始是随机的
initial_anchor_bee = tk.NW
initial_anchor_bee_x = rd.randrange(0, window_boundary_col - image_bee_width)
initial_anchor_bee_y = 0

# 己方战机 - 出现在窗口的下三分之一并水平居中
