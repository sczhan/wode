...
class ScreenSaver():
    ...
    def __init__(self):
        ...
        # 点击鼠标、移动鼠标、敲击键盘时退出程序
        # a = tkinter.messagebox.askquestion(title='hi', message='Are you sure to cancel it?')
        # b = tkinter.Button(self.root, text='there will be a msgbox',command=lambda:a)
        ...
        self.canvas = tkinter.Canvas(self.root, width=self.width, height=self.height, bg="white")
    ...
    # 这个弹窗可以省去,直接在myquit
    # # 弹窗
    # def hanshu(self, event):
    #     a = tkinter.messagebox.askyesno(title='退出', message='是否退出屏保?')
    #     if a:
    #          self.myquit(event)
    #     else:
    #         pass
